"""Rich terminal report rendering for the review summary."""
...
__all__ = ["render_review", "print_finding_table", "severity_style"]
...
_SEVERITY_ICON: dict[Severity, str] = {
    Severity.CRITICAL: "x", Severity.HIGH: "!", Severity.MEDIUM: "-", Severity.LOW: "·", Severity.INFO: "i",
}
...
def render_review(review: ReviewSummary, console: Console | None = None) -> None:
    ...
    header.add_row("Recommendation:", Text(rec_text, style=_REC_STYLE.get(review.recommendation, "white")))
    ...
    if review.pruned_chunks:
        header.add_row("Pruned:", Text(
            f"{len(review.pruned_chunks)} diff chunk(s) omitted for context budget "
            f"({', '.join(review.pruned_chunks[:5])}{' …' if len(review.pruned_chunks) > 5 else ''})",
            style="yellow",
        ))
