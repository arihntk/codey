...
class CacheDB:
    ...
    def has_call_edges(self, repo_path: str, git_hash: str) -> bool:
        """True when call OR import edges already exist for this (repo, hash).

        Both are built together by build_call_graph; checking only call_edges
        meant repos with imports but zero call edges (pure scripts) never
        satisfied the gate and import rows duplicated on every review.
        """
        ...
        if row:
            return True
        row = self._conn.execute(
            """SELECT 1 FROM import_edges WHERE repo_path=? AND git_hash=? LIMIT 1""",
            (repo_path, git_hash),
        ).fetchone()
        return row is not None
